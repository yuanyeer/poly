...
`MirrorExecutor` documents the future fill requirements and **rejects** intents
that would violate sleeve or risk caps. It never posts CLOB orders and never
appends ledger fills.

A real mirror (not implemented) must:

1. apply observed **delay** vs the leader fill
2. **walk book depth** (never top-of-book only)
3. apply CLOB **fees** (`fd.r` / `fd.to`)
4. refuse if sleeve or the existing 25% / 40% / ≤3 gates would fail
